import { Device, SDCardSpecs } from '../types/devices';
...
const deviceFunctionMap: Record<string, string> = {
  'Action Cameras': 'video recording',
  'Mirrorless Cameras': 'video recording',
  'Drones': '4K video recording',
  'Gaming Consoles': 'game loading',
};
const getDeviceFunction = (category: string): string => {
  return deviceFunctionMap[category] || 'recording';
};

const getCardTypeLabel = (type: string): string => {
  if (type.toLowerCase().includes('microsd')) return 'microSD';
  if (type.toLowerCase().includes('sd')) return 'SD';
  if (type.toLowerCase().includes('cfexpress')) return 'CFexpress';
  return type.split(' ')[0];
};

const extractCardFormat = (type: string): string => {
  if (type.toLowerCase().includes('microsd')) return 'microSD card';
  if (type.toLowerCase().includes('sd')) return 'SD card';
  if (type.toLowerCase().includes('cfexpress')) return 'CFexpress card';
  return 'memory card';
};

const hasMinSpeedRequirement = (minSpeed: string): boolean => {
  return !minSpeed.toLowerCase().includes('no minimum') && minSpeed !== 'Not specified';
};

const supportsUHSII = (type: string): boolean => {
  return type.toLowerCase().includes('uhs-ii') || type.toLowerCase().includes('uhs2');
};

const supportsUHSI = (type: string): boolean => {
  return !type.toLowerCase().includes('(not uhs-ii)') && !type.toLowerCase().includes('no uhs');
};

const isConsole = (category: string): boolean => {
  return category.includes('Console') || category.includes('Gaming');
};

const isCamera = (category: string): boolean => {
  return category.includes('Camera') || category.includes('mirrorless') || category.includes('action');
};

const isDrone = (category: string): boolean => {
 ...
 */
export function generateWhatCardNeededFAQ(device: Device): GeneratedFAQ | null {
  const { name, sdCard, category } = device;
  const cardFormat = extractCardFormat(sdCard.type);
  const hasMinSpeed = hasMinSpeedRequirement(sdCard.minSpeed);

  if (!hasMinSpeed) {
    return null; // Not applicable for devices without speed requirements
  }

  const highestCapacity = sdCard.recommendedCapacity[sdCard.recommendedCapacity.length - 1];
 ...
 */
export function generateMinimumSpeedFAQ(device: Device): GeneratedFAQ | null {
  const { name, sdCard, category } = device;
  const hasMinSpeed = hasMinSpeedRequirement(sdCard.minSpeed);
 ...
 */
export function generateCardTypeFAQ(device: Device): GeneratedFAQ | null {
  const { name, sdCard, category } = device;
  const cardType = getCardTypeLabel(sdCard.type);
  const hasMinSpeed = hasMinSpeedRequirement(sdCard.minSpeed);

  if (!cardType.includes('SD') && !cardType.includes('microSD')) {
    return null; // Only for SD/microSD cards
  }
 ...
 */
export function generateStorageFAQ(device: Device): GeneratedFAQ | null {
  const { name, sdCard, category } = device;
  const highestCapacity = sdCard.recommendedCapacity[sdCard.recommendedCapacity.length - 1];
  const maxCapacity = sdCard.maxCapacity;
 ...
 */
export function generateRecommendedBrandsFAQ(device: Device): GeneratedFAQ | null {
  const { name, category } = device;
  const context = isDrone(category)
 ...
 */
export function generateUHSInterfaceFAQ(device: Device): GeneratedFAQ | null {
  const { name, sdCard, category } = device;

  // Check if device type is relevant for UHS discussion
  if (!sdCard.type.toLowerCase().includes('uhs')) {
    return null;
  }

  const hasUHSII = supportsUHSII(sdCard.type);
  const onlyUHSI = !hasUHSII && sdCard.type.toLowerCase().includes('uhs-i');
 ...
 */
export function generateFAQs(device: Device): GeneratedFAQ[] {
 ...
 * Device-specific FAQs come first, then generated ones
 */
export function mergeFAQs(
  device: Device,
  deviceSpecificFAQs: GeneratedFAQ[] = []
): GeneratedFAQ[] {
